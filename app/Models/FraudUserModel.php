...
class FraudUserModel extends Model
{
    ...
    public function getFraudStatsByType(int $days = 30): array
    {
        ...
        return $this->select(
            "abuse_type, COUNT(*) as count, COUNT(DISTINCT user_id) as unique_users"
        )
    ...
    public function getTopFraudIPs(int $days = 7, int $limit = 20): array
    {
        ...
        return $this->select(
            "ip_address, COUNT(*) as count, COUNT(DISTINCT user_id) as unique_users"
        )
            ->where("created_at >=", $startDate)
